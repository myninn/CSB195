...
author: "Boris Steipe (with Wenzel Wohlgemut)"
date: last-modified      
version: "Test"
format:
  html:
...
hist(x[x>xi & x<xa], breaks = brk, col = "#0099FF33", add=TRUE)
...
hist(x[x>xi & x<xa], breaks = brk, col = "#00AABB44", add=TRUE)

# ... 500 ...
x <- x + rowSums(matrix(runif(n*500, -0.05, 0.05), nrow=n))
hist(x[x>xi & x<xa], breaks = brk, col = "#00CC9955", add=TRUE)

# ... and 1,000. That's our final histogram.
x <- x + rowSums(matrix(runif(n*1000, -0.05, 0.05), nrow=n))
hist(x[x>xi & x<xa], breaks = brk, col = "#00FF4466", add=TRUE)
```
...
myH <- hist(x[x>xi & x<xa], breaks = brk, col = "#00FF7744",
            freq = FALSE,
